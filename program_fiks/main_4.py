...
class App(ctk.CTk):
    ...
    def run_loop(self):
        ...
        with sr.Microphone() as source:
            self.after(0, lambda: self.result_box.configure(text="CALIBRATING..."))
            recognizer.adjust_for_ambient_noise(source, duration=2)

            while self.running:
                try:
                    self.after(0, lambda: self.result_box.configure(text="LISTENING..."))
                    ...
                    self.after(0, lambda: self.result_box.configure(text="PROCESSING..."))

                    text = recognizer.recognize_google(audio, language="id-ID")
                    result = classify_text(text)

                    self.after(0, lambda: self.result_box.configure(text=result))
                    ...
                    self.after(0, lambda: self.result_box.configure(text="NO VOICE"))
                    self.log_print("Timeout: tidak ada suara")

                except sr.UnknownValueError:
                    self.after(0, lambda: self.result_box.configure(text="TIDAK DIKENALI"))
                    self.log_print("Tidak dikenali")

                except sr.RequestError:
                    self.after(0, lambda: self.result_box.configure(text="API ERROR"))
                    self.log_print("API error")

                except Exception as e:
                    self.log_print(f"Error lain: {e}")
